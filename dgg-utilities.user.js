            ...
            Object.defineProperty(this, key, {
                ...
                get: function () {
                    ...
                    if (this[privateKeyName] === undefined) {
                        ...
                        this[privateKeyName] =
                            this.#load(keyName) ?? configKey.defaultValue;
                    }
...
function injectScript() {
    ...
    try {
        ...
        console.warn(
            `[WARNING] [dgg-utils] script might be running in cross-origin frame, can't get the live pill, the "change title on live" feature wont work - ${e}`
        );
    }
  ...
  .alertAnim {
                                ...
                                if (response.status == 200) {
                                    ...
                                    if ("link" in data && "version" in data) {
                                        if (
                                            ...
                                            chatlines.scrollTop =
                                                chatlines.scrollHeight;
                                        }
                                    }
                                } else {
                                    console.error(
                                        `[ERROR] [dgg-utils] couldn't check for updates - HTTP status code: ${response.status} - ${response.statusText}`
                                    );
                                }
                            },
                            onerror: () => {
                                console.error(
                                    `[ERROR] [dgg-utils] couldn't check for updates - HTTP error`
                                );
                            },
                            ontimeout: () => {
                                console.error(
                                    `[ERROR] [dgg-utils] couldn't check for updates - HTTP timeout`
                                );
                            },
                        });
    ...
    document
        .querySelector("#chat-output-frame")
    ...
    try {
        ...
        console.warn(
            `[WARNING] [dgg-utils] script might be running in cross-origin frame, can't get the og title of the stream, the "change title on live" feature wont work - ${e}`
        );
    }
    ...
    showLastVODLabel.innerHTML =
        "Show Destiny's last vod when you check embeds";
    ...
    youtubeEmbedFormatSelect.addEventListener(
        "change",
        () =>
            (config.youtubeEmbedFormat = parseInt(
                                ...
                                case "#youtube":
                                    GM.xmlHttpRequest({
                                        ...
                                        ontimeout: () => {
                                            errorAlert.style.display = "";
                                            console.error(
                                                `[ERROR] [dgg-utils] couldn't get the youtube oEmbed data - HTTP timeout`
                                            );
                                        },
                                    });
                                    break;
    ...
    function addFlairHidingStyles() {
        ...
        styleElem.innerHTML = config.hiddenFlairs.reduce(
            (prev, curr) =>
                prev + `.msg-chat .flair.${curr} { display: none; }\n`,
            ""
        );
    }
  ...
  `;
    ...
    ignoredPhrasesLabel.title =
        "Messages containing these phrases will be hidden";
    ...
    function toggleIgnoredPhrases() {
        document
            .getElementById("chat-win-main")
            .classList.toggle(
                "vyneer-util-ignored-phrases-on",
                config.ignorePhrases
            );
    ...
    ignoredPhrasesArea.addEventListener("change", () => {
        ...
        if (ignoredPhrasesArea.value.length > 0) {
            ...
            config.ignoredPhraseList =
                configItems.ignoredPhraseList.defaultValue;
        }
        toggleIgnoredPhrases();
    });
  ...
  `;
    ...
    class EmbedUrlFormatter {
        constructor() {
            ...
            try {
                ...
                console.warn(
                    `[WARNING] [dgg-utils] script might be running in cross-origin frame, can't get the bigscreen url, setting it to "https://www.destiny.gg/bigscreen" - ${e}`
                );
    ...
    class DGGMsg {
        constructor(str, type, stamp) {
            ...
            msgInnerTime.innerHTML = time.toLocaleTimeString(
                "en-US",
                timeOptions
            );
    ...
    function getPhrases() {
        ...
        GM.xmlHttpRequest({
                    ...
                    GM.xmlHttpRequest({
                        ...
                        onload: (response) => {
                            if (
                                ...
                                console.error(
                                    `[ERROR] [dgg-utils] couldn't get the phrase data - URL: "https://mitchdev.net/api/dgg/list", HTTP status code: ${response.status} - ${response.statusText}`
                                );
                            }
                        },
                        onerror: () => {
                        ...
                        ontimeout: () => {
                            console.error(
                                `[ERROR] [dgg-utils] couldn't get the phrase data - URL: "https://mitchdev.net/api/dgg/list", HTTP timeout`
                            );
                        },
                    });
                }
            ...
            onerror: () => {
                errorAlert.style.display = "";
                console.error(
                    `[ERROR] [dgg-utils] couldn't get the phrase data, going to try to get phrases from mitchdev.net - URL: "https://vyneer.me/tools/phrases", HTTP error`
                );
                GM.xmlHttpRequest({
                    method: "GET",
                    url: "https://mitchdev.net/api/dgg/list",
            ...
            ontimeout: () => {
                errorAlert.style.display = "";
                console.error(
                    `[ERROR] [dgg-utils] couldn't get the phrase data, going to try to get phrases from mitchdev.net - URL: "https://vyneer.me/tools/phrases", HTTP timeout`
                );
                GM.xmlHttpRequest({
                    method: "GET",
                    url: "https://mitchdev.net/api/dgg/list",
                    ...
                    GM.xmlHttpRequest({
                        ...
                        onload: (response) => {
                            ...
                            if (response.status == 200) {
                                ...
                                console.error(
                                    `[ERROR] [dgg-utils] couldn't get the embeds data - URL: "https://vyneer.me/tools/embeds/last", HTTP status code: ${response.status} - ${response.statusText}`
                                );
                            }
                        },
                        onerror: () => {
                            ...
                            console.error(
                                `[ERROR] [dgg-utils] couldn't get the embeds data - HTTP error`
                            );
                        },
                        ontimeout: () => {
                            new DGGMsg(
                            ...
                            console.error(
                                `[ERROR] [dgg-utils] couldn't get the embeds data - HTTP timeout`
                            );
                        },
                    });
                }
        ...
        GM.xmlHttpRequest({
            ...
            onload: (response) => {
                if (response.status == 200) {
                        ...
                        GM.xmlHttpRequest({
                            ...
                            onload: (response) => {
                                ...
                                if (response.status == 200) {
                                    ...
                                    console.error(
                                        `[ERROR] [dgg-utils] couldn't get the VOD data - HTTP status code: ${response.status} - ${response.statusText}`
                                    );
                                }
                                serveEmbeds(
                                    embedData,
                            ...
                            onerror: () => {
                                ...
                                console.error(
                                    `[ERROR] [dgg-utils] couldn't get the VOD data - HTTP error`
                                );
                            },
                            ontimeout: () => {
                                new DGGMsg(
                                ...
                                console.error(
                                    `[ERROR] [dgg-utils] couldn't get the VOD data - HTTP timeout`
                                );
                            },
                        });
                    } else {
                    ...
                    console.error(
                        `[ERROR] [dgg-utils] couldn't get the embeds data - URL: ${embedUrl}, HTTP status code: ${response.status} - ${response.statusText}`
                    );
                }
            },
            onerror: () => {
    ...
    function getNukesAndLinks() {
        GM.xmlHttpRequest({
            ...
            onload: (response) => {
                ...
                if (response.status == 200) {
                    ...
                    if (data.length > 0) {
                        ...
                        data.forEach((entry) => {
                            ...
                            if (
                                ...
                                const regexString = cleanNuke.slice(
                                    1,
                                    cleanNuke.length - 1
                                );
                                const regex = new RegExp(regexString, "i");
                    ...
                    console.error(
                        `[ERROR] [dgg-utils] couldn't get the nuke data - HTTP status code: ${response.status} - ${response.statusText}`
                    );
                }
            },
            onerror: () => {
                errorAlert.style.display = "";
                console.error(
                    `[ERROR] [dgg-utils] couldn't get the nuke data - HTTP error`
                );
            },
            ontimeout: () => {
                errorAlert.style.display = "";
                console.error(
                    `[ERROR] [dgg-utils] couldn't get the nuke data - HTTP timeout`
                );
            },
        });
        ...
        GM.xmlHttpRequest({
            ...
            onerror: () => {
                errorAlert.style.display = "";
                console.error(
                    `[ERROR] [dgg-utils] couldn't get the mutelinks data - HTTP error`
                );
            },
            ontimeout: () => {
                errorAlert.style.display = "";
                console.error(
                    `[ERROR] [dgg-utils] couldn't get the mutelinks data - HTTP timeout`
                );
            },
        });
    }
    ...
    nukeAlertButton.addEventListener("click", () => {
        ...
        if (nukes.length > 0) {
            nukes.forEach((result) => {
                new DGGMsg(
                    `${result.word} (${result.type
                        .toString()
                        .toLowerCase()}d for ${result.duration})`,
                    "msg-status msg-historical",
                    ""
                ).update();
            });
